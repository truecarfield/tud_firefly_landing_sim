...
#include "tud_firefly_gazebo_plugins/gazebo_motor_model_mod.h"

namespace gazebo {

GazeboMotorModelMod::~GazeboMotorModelMod() {
...
void GazeboMotorModelMod::InitializeParams() {}

void GazeboMotorModelMod::Publish() {
  turning_velocity_msg_.data = joint_->GetVelocity(0);
  motor_velocity_pub_.publish(turning_velocity_msg_);
}

void GazeboMotorModelMod::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  model_ = _model;
  ...
  updateConnection_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&GazeboMotorModelMod::OnUpdate, this, _1));

  command_sub_ = node_handle_->subscribe(command_sub_topic_, 1, &GazeboMotorModelMod::VelocityCallback, this);
  wind_speed_sub_ = node_handle_->subscribe(wind_speed_sub_topic_, 1, &GazeboMotorModelMod::WindSpeedCallback, this);
...
void GazeboMotorModelMod::OnUpdate(const common::UpdateInfo& _info) {
...
void GazeboMotorModelMod::VelocityCallback(const mav_msgs::ActuatorsConstPtr& rot_velocities) {
  boost::mutex::scoped_lock lock(command_mutex_);
 ...
 // if (motor_number_==1) {std::cout<<"Actuator heard the sequence "<<rot_velocities->header.seq<<std::endl;}// by ZD
  ref_motor_rot_vel_ = std::min(rot_velocities->angular_velocities[motor_number_], max_rot_velocity_);
}

void GazeboMotorModelMod::WindSpeedCallback(const rotors_comm::WindSpeedConstPtr& wind_speed) {
 boost::mutex::scoped_lock lock(command_mutex_);
...
void GazeboMotorModelMod::UpdateForcesAndMoments() {
...
GZ_REGISTER_MODEL_PLUGIN(GazeboMotorModelMod);
}
