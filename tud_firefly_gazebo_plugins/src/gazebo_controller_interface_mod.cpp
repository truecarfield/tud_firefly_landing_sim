...
#include "tud_firefly_gazebo_plugins/gazebo_controller_interface_mod.h"

namespace gazebo {

GazeboControllerInterfaceMod::~GazeboControllerInterfaceMod() {
...
void GazeboControllerInterfaceMod::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  updateConnection_ = event::Events::ConnectWorldUpdateBegin(
      boost::bind(&GazeboControllerInterfaceMod::OnUpdate, this, _1));

  cmd_motor_sub_ = node_handle_->subscribe(command_motor_speed_sub_topic_, 1,
                                           &GazeboControllerInterfaceMod::CommandMotorCallback,
                                           this);
...
void GazeboControllerInterfaceMod::OnUpdate(const common::UpdateInfo& /*_info*/) {
  ...
  // std::cout<<"Controller update the sound of "<<turning_velocities_msg->angular_velocities[0]<<std::endl;
  motor_velocity_reference_pub_.publish(turning_velocities_msg);
}

void GazeboControllerInterfaceMod::CommandMotorCallback(const mav_msgs::ActuatorsConstPtr& input_reference_msg) {
 boost::mutex::scoped_lock lock(command_mutex_);
  // std::cout<<"Controller heard the sequence of "<<input_reference_msg->header.seq<<std::endl;
...
GZ_REGISTER_MODEL_PLUGIN(GazeboControllerInterfaceMod);
}
