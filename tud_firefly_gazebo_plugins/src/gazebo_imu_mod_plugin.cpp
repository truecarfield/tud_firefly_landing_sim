...
#include "tud_firefly_gazebo_plugins/gazebo_imu_mod_plugin.h"
...
GazeboImuModPlugin::GazeboImuModPlugin()
    : ModelPlugin(),
      node_handle_(0),
...
void GazeboImuModPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  this->updateConnection_ =
      event::Events::ConnectWorldUpdateBegin(
          boost::bind(&GazeboImuModPlugin::OnUpdate, this, _1));
...
void GazeboImuModPlugin::addNoise(Eigen::Vector3d* linear_acceleration,
...
void GazeboImuModPlugin::OnUpdate(const common::UpdateInfo& _info) {
  ...
  math::Vector3 acceleration_I =
      C_W_I.RotateVectorReverse(acceleration);// - gravity_W_);  // by ZD
...
//  imu_message_.orientation.z = C_W_I.z;
  ...
  //std::cout<<"imu update time is "<<imu_message_.header.stamp.toSec()<<std::endl;// by ZD
  imu_pub_.publish(imu_message_);
}

GZ_REGISTER_MODEL_PLUGIN(GazeboImuModPlugin);
}
