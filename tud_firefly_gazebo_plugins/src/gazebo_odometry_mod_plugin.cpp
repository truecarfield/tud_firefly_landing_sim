#include "tud_firefly_gazebo_plugins/gazebo_odometry_mod_plugin.h"
...
GazeboOdometryModPlugin::~GazeboOdometryModPlugin() {
...
// void GazeboOdometryModPlugin::InitializeParams() {};
// void GazeboOdometryModPlugin::Publish() {};

void GazeboOdometryModPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  getSdfParam<std::string>(_sdf, "odometryFrameId", odometry_frame_id_, odometry_frame_id_); // by ZD
  getSdfParam<std::string>(_sdf, "worldFrameId", world_frame_id_, world_frame_id_); // by ZD
  getSdfParam<std::string>(_sdf, "parentFrameId", parent_frame_id_, parent_frame_id_); // by ZD
  ...
  parent_link_ = world_->GetEntity(world_frame_id_);  //  by ZD
  if (parent_link_ == NULL && world_frame_id_ != kDefaultParentFrameId) {
    gzthrow("[gazebo_odometry_plugin] Couldn't find specified parent link \"" << world_frame_id_ << "\".");
  }
  ...
  updateConnection_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&GazeboOdometryModPlugin::OnUpdate, this, _1));
...
void GazeboOdometryModPlugin::OnUpdate(const common::UpdateInfo& _info) {
  // C denotes child frame, P parent frame, and W world frame.
  // Further C_pose_W_P denotes pose of P wrt.(with respect to) W expressed in C.
  ...
  if (world_frame_id_ != kDefaultWorldFrameId) {
  ...
  if (gazebo_sequence_ % measurement_divisor_ == 0) {
    ...
    odometry.header.frame_id = odometry_frame_id_;
  ...
  if (gazebo_sequence_ == odometry_queue_.front().first) {
    ...
    // std::cout << "published odometry with seq " << odometry->header.seq << "at time t " << world_->GetSimTime().Double()<<std::endl;
  }

  ++gazebo_sequence_;
}

GZ_REGISTER_MODEL_PLUGIN(GazeboOdometryModPlugin);
}
