...
void LandingController::ComputeDesiredAcceleration(Eigen::Vector3d* acceleration) {
  ...
  else
  {
    ...
    *acceleration << pidX_.update_f(velocity_error(0), std::numeric_limits<double>::quiet_NaN(), dt_.toSec()); // - cmd_accel_N_(0),
                                      pidY_.update_f(velocity_error(1), std::numeric_limits<double>::quiet_NaN(), dt_.toSec()); // - cmd_accel_N_(1),
                                      pidZ_.update_f(velocity_error(2), std::numeric_limits<double>::quiet_NaN(), dt_.toSec()) - vehicle_parameters_.gravity_*e_3(2); // - cmd_accel_N_(2);
                                                     ...
                                                     Eigen::Vector3d* angular_acceleration)
{
  ...
  if (use_R_des_tracker_)
  {
    // Define R_des differential tracker variables for this iteration.
    Eigen::VectorXd RPY_c(3), dot_RPY_c_f(3);
    // Define the matrix form of dot_RPY_c.
    Eigen::Matrix3d  dot_R_des_f;

    RPY_c << cmd_roll_, cmd_pitch_, cmd_yaw; // it should be cmd_yaw_
    R_des_tracker_.diff(RPY_c, dot_RPY_c_f, dt_.toSec());

    // std::cout<<"dot rollf:"<<dot_RPY_c_f(0) <<", dot pitchf:"<<dot_RPY_c_f(1)<<", dot yawf:"<<dot_RPY_c_f(2)<<std::endl;
    angular_rate_des = W_.inverse()*dot_RPY_c_f;
  }
  else
  {
    angular_rate_des[2] = command_trajectory_.getYawRate();
    Eigen::VectorXd RPY_c(3);
    RPY_c << cmd_roll_, cmd_pitch_, cmd_yaw;
    R_des_tracker_.reportX(RPY_c);
  }

  Eigen::VectorXd dot_angular_rate_des_f(3);
  if (use_rate_des_tracker_)
  {
    angular_rate_des_tracker_.diff(angular_rate_des, dot_angular_rate_des_f, dt_.toSec());
  }
  else
  {
    dot_angular_rate_des_f.setZero();
  }
  ...
  // std::cout<<"cmd_yaw:"<<cmd_yaw<<", cmd_yaw_rate"<<angular_rate_des(2)<<std::endl;
